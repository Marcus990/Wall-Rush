...
function onTouched(hit)
	if hit and hit.Parent and hit.Parent:FindFirstChildOfClass("Humanoid") and not debounce then
		...
		local player = Players:GetPlayerFromCharacter(hit.Parent)
			game.ReplicatedStorage.Events:WaitForChild("ChangeColorAndPlayFinishedSound"):FireClient(player, "Checkpoint1")
		game.ReplicatedStorage.Events.Checkpoint1Touched:FireClient(player)
