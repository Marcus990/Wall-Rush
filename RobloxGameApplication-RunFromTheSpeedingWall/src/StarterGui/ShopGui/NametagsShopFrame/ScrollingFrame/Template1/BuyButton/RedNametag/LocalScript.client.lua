...
--local NametagEquipVariablesTable = require(game.ReplicatedStorage.ModuleScript)
--local RedNametagEquipVariablesTable = NametagEquipVariablesTable.RedNametagEquipVariablesTable
local function whatdoicallthisfunctionversion2(stringvalue)
	...
	elseif stringvalue == "Unequip" then
		button.Parent.ImageColor3 = Color3.fromRGB(252, 1, 7)
		button.Text = "Unequip"
		player.Character.Head:FindFirstChild("newtext").UpperText.TextColor3 = Color3.fromRGB(252, 1, 7)
	end
end
...
button.MouseButton1Down:Connect(function()
	...
	if button.Text == "Equip" then
		if script.Parent.Parent.Parent.Parent.Template2.BuyButton.OrangeNametag.Text == "Unequip" then
	...
	elseif button.Text == "Unequip" then
		if player.Character.Head:FindFirstChild("newtext").UpperText.TextColor3 == Color3.fromRGB(252, 1, 7) then
	...
	elseif player.leaderstats.Coins.Value >= 50 and button.Text == "Buy" then
		...
		wait(2)
		button.Text = "Equip"
		TrailForDatastoreEvent:FireServer(22, game.Players.LocalPlayer)
	elseif player.leaderstats.Coins.Value < 50 and button.Text == "Buy" then
...
RedNametagForDatastoreEvent.OnClientEvent:Connect(whatdoicallthisfunctionversion2)
