...
local Trail = "RedTrail"
player.CharacterAdded:Connect(function(char)
	local humanoid = char:WaitForChild("Humanoid")
	humanoid:GetPropertyChangedSignal("Health"):Connect(function()
		if humanoid.Health <= 0 then
			local text
			if player:WaitForChild("PlayerGui"):WaitForChild("ShopGui").TrailsShopFrame.ScrollingFrame.Template.BuyButton.RedTrail.Text == "Unequip" then 
				text = true
			elseif player:WaitForChild("PlayerGui"):WaitForChild("ShopGui").TrailsShopFrame.ScrollingFrame.Template.BuyButton.RedTrail.Text == "Equip" then
				text = false
			end
